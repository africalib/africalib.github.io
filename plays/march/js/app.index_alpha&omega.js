...
app.controller('appCtrl', function ($scope, $timeout, $interval) {
    ...
    let local = {
		...
		getIsInShelterOrArea: function(i) {
			if(local.getIsUnitInArea(i)) {			
				...
				return eachUnit.player === $scope.getPlayer() && (eachArea.owner === $scope.getPlayer() || (local.getIsShelterInArea(i) && eachArea.shelter.player === $scope.getPlayer()));
			}
            ...
            while (isBlackTurn ? i > 0 : i < $scope.areas.length) {
                ...
                if (this.getIsUnitInArea(idx) && eachArea.unit.player !== $scope.status.turn && eachArea.unit.distance && eachArea.unit.power) {
                    ...
                    for (let j = 0; j < eachUnit.distance + eachUnit.buffed['distance']; j += 1) {
                        ...
                        if (targetIdx && !this.getIsShelterInArea(targetIdx) && targetIdx === $scope.active.idx && targetIdx >= 0 && this.getIsUnitInArea(targetIdx) && eachUnit.player !== $scope.areas[targetIdx].unit.player && this.getIsInCross(idx, targetIdx)) {
                            ...
                            else if(eachArea.unit.type === $scope.areas[targetIdx].unit.type) {
        ...
        setAttack: function (targetIdx, stay, delay) {
            ...
            if (((this.getIsShelterInArea(targetIdx) && targetArea.shelter.player !== activeArea.unit.player) || (this.getIsUnitInArea(targetIdx) && targetArea.unit.player !== activeArea.unit.player)) && this.getIsUnitInArea($scope.active.idx)) {
                ...
                let targetDirection = this.getDirection($scope.active.idx, targetIdx);
                ...
                if (this.getIsShelterInArea(targetIdx)) {
                    ...
                    if (activeArea.unit.hp < 1) {
                        ...
                        targetArea.unit.exp += activeArea.unit.crop;
                    }

                    if (targetArea.unit.hp < 1) {
                        targetArea.unit.hp = 0;
                        activeArea.unit.exp += targetArea.unit.crop;
                    ...
                    else if (activeArea.unit.distance > 1) {
                        local.autoRotateArr.push({ idx: targetIdx, direction: targetDirection });
                    }
        ...
        setTurn: function (player) {
            ...
            for (let i in $scope.areas) {
                ...
                if (eachUnit.name && eachUnit.player === player) {
                    let inInBuffArr = false;
                    let restoreHp = 0;
		...
		if(local.getIsUnitInArea(rideIdx)) {
			...
			if(rideUnit.ridable && rideUnit.rided.length) {
				...
				for(let i = 1; i < $scope.default.columNum; i += 1) {
					if(local.getIsUnitInArea(rideIdx - 1) && !local.getHasUnit(rideIdx - 1)) {
    ...
    $scope.pass = function (player, isPosted) {
        if ($scope.status.turn !== player) {
			...
			if(!$scope.status[player].team) {
				if(confirm('알파팀을 선택하시려면 확인, 오메가팀을 선택하시려면 취소를 눌러주세요.')) {
					$scope.status[player].team = 'alpha';
				}
				else {
					$scope.status[player].team = 'omega';
				}
				...
				for(let i in $scope.areas) {
					...
					if(player === 'white') {
						...
						if (i >= 123 && i <= 125)
							...
							local.setUnit('black', 'elephant', i, true);
						else if (i === 134)
                ...
                for (let i = 0; i < movePoint; i += 1) {
                    ...
                    for (let j = 0; j < 4; j += 1) {
                        ...
                        if (each.cond && eachArea) {
                            ...
                            if (attackable && ((local.getIsShelterInArea(each.idx) && !local.getHasShelter(each.idx)) || (local.getIsUnitInArea(each.idx) && !local.getHasUnit(each.idx))) && targetArea.unit.distance === 1)
            ...
            if ($scope.areas[$scope.active.idx].unit.distance === 1 || ($scope.areas[$scope.active.idx].unit.distance > 1 && targetArea.status === 'move')) {
                ...
                for (let i in obj.loopArr) {
                    ...
                    if (loopIdx !== $scope.active.idx) {
                        if ((local.getIsShelterInArea(loopIdx) && !local.getHasShelter(loopIdx)) || local.getIsUnitInArea(loopIdx)) {
                            ...
                            else if (local.setAttack(loopIdx, false, i > 1)) {
                ...
                for (let i in teamUnits) {
                    if (local.getIsUnitInArea(teamUnits[i].idx)) {
                        ...
                        for (let j in obj.loopArr) {
                            if (!local.getIsUnitInArea(obj.loopArr[j]) && !local.getIsShelterInArea(obj.loopArr[j])) {
                                $scope.areas[obj.loopArr[j]].unit = teamUnits[i].unit;
                                break;
                            }
                        }
                    }
        ...
        if (!$scope.status.started) {
            ...
            mc.on('press', function (e) {
                ...
                if (eachArea.length && eachArea.data('idx')) {
                    ...
                    if ($scope.areas[idx] && $scope.areas[idx].shelter) {
    ...
    $scope.grab = function (player, name, isPosted) {
        if ($scope.status.turn === player) {
            ...
            for (let i in $scope.areas) {
                let area = $scope.areas[i];
                if (!area.unit.name && unit.type === area.type) {
                    switch (player) {
                        case 'white':
                            if (area.owner === 'white')
                                area.status = 'enter';
