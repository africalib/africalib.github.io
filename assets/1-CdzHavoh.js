import{a as l,d as n,m as t,F as i,o as s}from"./index-CsHA_pvf.js";const p={},m=`<p>라우터는 현재 URL에 맞는 페이지를 연결해주는 깜찍한 녀석입니다. 예를 들어, 사용자가 로그인 URL로 접속하면 로그인 페이지를 연결하고요, 장바구니 URL로 접속하면 장바구니 페이지를 연결해주는 것이죠. 마치 영화 감독이 상황에 따라 어떤 장면을 촬영할 것인지 결정하는 것과 비슷합니다. 라우터 코드를 통해 더 구체적으로 살펴볼까요?</p>
...
<pre><code class="language-txt">* 일반적으로 HTML5 모드를 권장합니다. 해시 모드는 검색 엔진 최적화(SEO)에 좋지 않습니다.
</code></pre>
<ul>
<li>URL과 컴포넌트 연결
<ul>
...
`,c=`라우터는 현재 URL에 맞는 페이지를 연결해주는 깜찍한 녀석입니다. 예를 들어, 사용자가 로그인 URL로 접속하면 로그인 페이지를 연결하고요, 장바구니 URL로 접속하면 장바구니 페이지를 연결해주는 것이죠. 마치 영화 감독이 상황에 따라 어떤 장면을 촬영할 것인지 결정하는 것과 비슷합니다. 라우터 코드를 통해 더 구체적으로 살펴볼까요?\r
\r
\r
\`\`\`js\r
...
\r
\`\`\`txt\r
* 일반적으로 HTML5 모드를 권장합니다. 해시 모드는 검색 엔진 최적화(SEO)에 좋지 않습니다.\r
\`\`\`\r
\r
- URL과 컴포넌트 연결\r
...
\`\`\``;function o(r,e){return s(),l(i,null,[e[0]||(e[0]=n("p",null,"라우터는 현재 URL에 맞는 페이지를 연결해주는 깜찍한 녀석입니다. 예를 들어, 사용자가 로그인 URL로 접속하면 로그인 페이지를 연결하고요, 장바구니 URL로 접속하면 장바구니 페이지를 연결해주는 것이죠. 마치 영화 감독이 상황에 따라 어떤 장면을 촬영할 것인지 결정하는 것과 비슷합니다. 라우터 코드를 통해 더 구체적으로 살펴볼까요?",-1)),e[1]||(e[1]=n("pre",null,[n("code",{class:"language-js","v-pre":"true"},`import Home from '../views/Home.vue';
...
`)],-1)),e[2]||(e[2]=n("ul",null,[n("li",null,[t("히스토리 모드 "),n("ul",null,[n("li",null,"createWebHistory: HTML5 모드입니다. (예: example.com/user/1)"),n("li",null,"createWebHashHistory: 해시 모드입니다. (예: example.com/#/user/1)")])])],-1)),e[3]||(e[3]=n("pre",null,[n("code",{class:"language-txt","v-pre":"true"},`* 일반적으로 HTML5 모드를 권장합니다. 해시 모드는 검색 엔진 최적화(SEO)에 좋지 않습니다.
